...
| `c0cd9fe` | `bootstrapNraApplication` now sets `keepAliveTimeout` (35s) and `headersTimeout` (36s) on the underlying HTTP server, to avoid Caddy reusing a keep-alive connection Node already closed | Add `caddy.reverse_proxy.transport: http` and `caddy.reverse_proxy.transport.keepalive: 30s` labels to the `server` service's `deploy.labels` in `docker-compose-swarm.yml` (must stay below Node's 35s `keepAliveTimeout`) | |
