...
| `28a61f5` | `TestAppHelper` now wires `cookie-parser` itself; `utils/testing/e2e/test-utils.ts` adds `registerAndAuthenticate()` (register a test user + guarded Authentication-cookie extraction) and `asList()` (normalizes a CRUD list response that may be a plain array or `{ data: [...] }`) | Not required to keep working (purely additive), but each app's `server/test/*-crud.e2e-spec.ts` should swap its hand-rolled `Test.createTestingModule`/cookie-parser/cookie-regex/teardown boilerplate for `createTestApp()` + `registerAndAuthenticate()` + `asList()` to drop the duplication | none yet |
