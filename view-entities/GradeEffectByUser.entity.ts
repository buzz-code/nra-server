...
@ViewEntity('grade_effect_by_user', {
    expression: (dataSource: DataSource) => dataSource
        ...
        .leftJoin(AttGradeEffect, 'att_grade_effect', 'att_grade_effect.user_id = users.id AND att_grade_effect.percents <= numbers.number')
        .groupBy('users.id')
        .addGroupBy('numbers.number')
        .orderBy('users.id')
        .addOrderBy('numbers.number')
})
export class GradeEffectByUser implements IHasUserId {
    @PrimaryColumn()
    id: string;
