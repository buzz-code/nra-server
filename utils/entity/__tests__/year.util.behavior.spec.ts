...
describe('Year Util Behavior Tests - Current Logic', () => {
    ...
    const testCases = [
        // Before August 1st - should use current Hebrew year
        { date: '2025-07-30T12:00:00Z', description: '30 July 2025 (before academic year)', expected: 5785 },
        { date: '2025-07-31T23:59:59Z', description: '31 July 2025 (last day before academic year)', expected: 5785 },

        // After August 1st - should use next Hebrew year (academic year logic)
        { date: '2025-08-01T00:00:00Z', description: '1 August 2025 (start of academic year)', expected: 5786 },
        ...
        { date: '2026-07-31T23:59:59Z', description: '31 July 2026 (before next academic year)', expected: 5786 },
        { date: '2026-08-01T00:00:00Z', description: '1 August 2026 (start of next academic year)', expected: 5787 },

        // Edge cases around different years
        { date: '2024-08-01T12:00:00Z', description: '1 August 2024', expected: 5785 },
        { date: '2024-07-31T12:00:00Z', description: '31 July 2024', expected: 5784 },
    ...
    describe('August 1st boundary cases', () => {
        it('should handle exact midnight on August 1st', () => {
            jest.spyOn(global.Date, 'now').mockImplementation(() =>
                new Date('2025-08-01T00:00:00Z').valueOf()
            );

            const result = getCurrentHebrewYear();
            expect(result).toBe(5786);
            console.log(`Exactly midnight Aug 1st 2025: ${result}`);
        });

        it('should handle one second before midnight on July 31st', () => {
            jest.spyOn(global.Date, 'now').mockImplementation(() =>
                new Date('2025-07-31T23:59:59Z').valueOf()
            );

            const result = getCurrentHebrewYear();
            expect(result).toBe(5785);
            console.log(`One second before midnight July 31st 2025: ${result}`);
        });
    });
});